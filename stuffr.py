...
def sec2dirname_jm(t):
    return(unix2date(t).strftime("%Y%m%d_%H"))
...
def unix2datestr_jm(x):
    fs=numpy.floor(x)
    frac_s=x-fs
    sstr="%1.4f"%(frac_s)
    sstr=sstr[1:6]
    return("%s%s"%(unix2date(x).strftime('%Y %m %d %H %M %S'),sstr))
...
def decimate_max(v,dec=10):
    v2=numpy.zeros(len(v)/dec)
    for i in range(len(v2)):
        v2[i] = numpy.max(v[(i*dec):((i+1)*dec)])
    return(v2)
